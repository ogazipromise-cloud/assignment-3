...
grep -Eq 'validate:' "$WORKFLOW" \
    && pass "Workflow has validate job" \
    || fail "Workflow missing validate job"

grep -Eq 'test:' "$WORKFLOW" \
    && pass "Workflow has test job" \
    || fail "Workflow missing test job"

grep -Eq 'docker:' "$WORKFLOW" \
    && pass "Workflow has docker job" \
    || fail "Workflow missing docker job"

grep -A12 -E '^[[:space:]]*test:' "$WORKFLOW" |
    grep -Eq 'needs:[[:space:]]*validate' \
    && pass "Test job depends on validate" \
    || fail "Test job should use needs: validate"

grep -A12 -E '^[[:space:]]*docker:' "$WORKFLOW" |
    grep -Eq 'needs:[[:space:]]*test' \
